...
// LsLocal structure
type LsLocal struct {
	Cipher     *myproxy.Cipher
	ListenAddr *net.TCPAddr
	RemoteAddr *net.TCPAddr
}

// NewLsLocal - create a new local
// 新建一个本地端
// 本地端的职责是:
// 1. 监听来自本机浏览器的代理请求
// 2. 转发前加密数据
...
func NewLsLocal(password string, listenAddr, remoteAddr string) (*LsLocal, error) {
	bsPassword, err := myproxy.ParsePassword(password)
	if err != nil {
		return nil, err
	}
	...
	return &LsLocal{
		Cipher:     myproxy.NewCipher(bsPassword),
...
func (local *LsLocal) Listen(didListen func(listenAddr *net.TCPAddr)) error {
	return myproxy.ListenEncryptedTCP(local.ListenAddr, local.Cipher, local.handleConn, didListen)
}

func (local *LsLocal) handleConn(userConn *myproxy.SecureTCPConn) {

	defer userConn.Close()

	proxyServer, err := myproxy.DialEncryptedTCP(local.RemoteAddr, local.Cipher)
	if err != nil {
		log.Println(err)
		return
	}
